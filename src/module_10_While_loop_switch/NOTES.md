...
// to Check if there is input available
...
return;}
```
* Missing Quotes on String Literal (Compile-time Error):
 18 question learning
* case (circle)-> treats circle as an undeclared variable. In Java, switch case labels for strings must be enclosed in double quotes without parentheses: case "circle" ->.


* Key ConsiderationsWhitespace: scanner.next() automatically skips leading whitespace before capturing your input token.Primitive Type: Extracting with .charAt(0) gives you a primitive char type, which is memory efficient and perfect for switch statements or conditional checks.

* In switch -> Variable ch is of type char.
case "a", "e", "i", "o", "u" uses double quotes (" "), which represents String literals. For char, you must use single quotes (' '):
...
* **No variables:** The case value must be a literal or constant.
* **No duplicates:** No two cases should be of same value. Otherwise, a compilation error is thrown.
* **Allowed Types:** `int`, `byte`, `short`, `char`, and `String` type. Primitives are allowed with their wrapper types.
* **Optional Break Statement:** Break statement is optional. If a case is matched and there is no break statement mentioned, subsequent cases are executed until a break statement or end of the switch statement is encountered (fall-through condition).
* **Optional default case:** Default case value is optional. The default statement is meant to execute when there is no match between the values of the variable and the cases. It can be placed anywhere in the switch block.
